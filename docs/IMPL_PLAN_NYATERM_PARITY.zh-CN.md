...
- **Task P2-2 会话导入（4）**：`backend/src/connection_import.rs` 解析器（Moba INI → Xshell ZIP+INI → WindTerm JSON+PBKDF2-SHA3-512/AES-CBC，主密码缺失明确报错）；主文件与可选 `user.config` 使用 `import/preview/start|finish|cancel` + offset 二进制分块/ACK，64 MiB 总预算、任务数/TTL/进程内存门限，取消/失败/完成/过期均清理并零化源字节与主密码；仅返回预览和脱敏规范化导出，永不持久化会话或凭据。启动时删除旧 `<plugin_data_dir>/imported-connections.json`，不读取其内容；UI 向导入 SideNavPanel；zip-bomb 防护（条目/解压上限）；Web/Docker 走 File API 上传字节。
