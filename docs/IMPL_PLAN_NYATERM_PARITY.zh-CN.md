...
- **Task P2-2 会话导入（4）**：`backend/src/connection_import.rs` 解析器（Moba INI → Xshell ZIP+INI → WindTerm JSON+PBKDF2-SHA3-512/AES-CBC，主密码缺失明确报错）；这是**临时 preview/export**，不创建连接、永不持久化会话或凭据。主文件与可选 `user.config` 使用 `import/preview/start|finish|cancel` + offset 二进制分块/ACK，64 MiB 总预算、任务数/TTL/进程内存门限，取消/失败/完成/过期均清理并零化源字节与主密码；解析前受限 accumulator 在各格式（含 ZIP）`push` 前执行 `MAX_PREVIEW_SESSIONS` 预算，超限 fail-closed。预览模型仅含认证种类、`hasSecret`、非敏感密钥路径及原因元数据；普通密码、私钥内容和口令仅在 `Zeroizing` 临时缓冲内检查并立即清零。启动时删除旧 `<plugin_data_dir>/imported-connections.json`，不读取其内容；UI 向导入 SideNavPanel；zip-bomb 防护（条目/解压上限）；导出优先 `saveFile`/`fileTransfer`，Host API 1.0 缺失两者时仅顶层非 sandbox 走 Blob 下载，iframe 按 issue #93 明确指引升级宿主或改在顶层打开。
