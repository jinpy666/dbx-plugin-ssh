...
| 文件夹上传（递归目录上传、保留结构） | Tabby/WindTerm 目录拖放 | ✅ 已有（2026-09-26，issue #78 / M31-A：见上「终端拖入文件上传」行内文件夹上传段——纯前端编排，`sftp/createDirectory` 父先于子 ensure + `sftp/upload/start/finish` 既有管线；`transfer_duplicate_policy=ask` 批量下降级为已存在跳过并计数提示，rename/overwrite 走既有解析；聚合进度卡「目录 X/Y · 文件 N/M · 字节」复用传输面板；能力探测 webkitdirectory 缺失时入口隐藏） | — |
