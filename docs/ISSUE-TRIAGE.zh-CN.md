...
| 95 | 终端执行 git 看不到内容 | **取证完成：非插件数据通路缺陷**（M31-D，2026-09-26） | 用本地容器 + sidecar_client 建 PTY 会话实测（证据链见 `docs/PROGRESS-P-SSH.zh-CN.md` M31-D）：`git --version` / `git status` / `git log`（含 less pager 渲染帧）在 `ssh/terminal/out` 二进制帧上**字节级完好**；前端输出管线逐环节审计（OSC 633 parser / directory parser 均为旁路观察、写节流与背压门字节保持、sentry 与 trzsz filter 空闲透传）无吞字节环节。issue 截图重新判读：标题/提示/页脚全部完整，"红色碎片"实为 TAB 缩进的**完整行**——即 git 实际列出的文件名（`'`、`i`、`0,`、`[`、`true,`，JSON/Python 字面量经 shell 分词产生的文件名特征） | 用户的仓库里确实存在这些垃圾文件名，git 如实打印，终端如实渲染；"看不到内容"是对显示内容的困惑而非输出丢失。最可能根因：更早一条未加引号的 shell 命令（或粘贴事故）把 JSON/列表字面量按空白分词创建成了多个文件。**不做猜测性代码修复**；建议回复提问者核实仓库内未跟踪文件列表（`ls -la`） |
...
| 90 | sz 命令下载无反应（enhancement） | **反馈层已交付（M31-D，2026-09-26）；ZMODEM 直传支持仍是独立决策** | sidecar 在 SSH PTY 输出泵挂 ZRQINIT 触发检测（`backend/src/zmodem_detect.rs` 纯状态机 + 9 单测含跨帧边界）：命中即抑制协议帧进终端（满屏乱码/静默 deny 都不再发生）并发 `ssh/zmodem` 事件，前端浮出「建议使用 SFTP 面板下载」七语提示（`zmodemDownloadUnsupported`）；ZRINIT（rz 上传）原样透传，既有上传流程不受影响；40s 抑制窗口覆盖 sz 的 ~30s 重试生命周期后自动复位 | 验证：`scripts/smoke_zmodem_detect.py` 四用例全 PASS（sz ZRQINIT 抑制 + 事件恰一次 + 窗口内重试静默 + ZRINIT 透传 + 字面 `**` 文本无损）；协议语义见 `docs/PROTOCOL.zh-CN.md`「ZMODEM 触发检测」节。**边界**：不实现 ZMODEM 协议（不应答 ZRINIT、不收发文件）——"sz 直传下载"若要做需独立立项（接收方向状态机 + 进度 UI），#65 为该条重复项 |
