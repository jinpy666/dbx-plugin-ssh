...
3. **登录期 keyboard-interactive 无兜底指路**：无 TOTP secret 时服务器只发 KI
   ...
   **2026-09-17 表单信息架构（同一问题的第三轮）**：`sudo_source` 与
   ...
   > ⚠️ 状态标注（2026-09-27 合并 ssh-form 设计线时补记）：以下第四~六轮为并行设计线的
> 方案推演，**未并入集成分支的 manifest**（集成分支表单以 M32 收口状态为准：engines.dbx
> >=0.6.21、M9/M32-B protocol 化与 serial/rdp 字段组）。保留作设计史，落地需独立 PR 评审。
